<x-base-layout>
    <main class="fixture-edit">
        <div style="margin-bottom: 20px;">
            <a href="{{ route('tournaments.show', $fixture->tournament_id) }}" class="btn-goback" style="display: inline-block; margin-bottom: 15px;">← Terug naar wedstrijden</a>
        </div>
        ...
        <h2>{{ $fixture->team1->name }} vs {{ $fixture->team2->name }}</h1>
            <h3>{{ $fixture->type  }}</h1>
                ...
                <form action="{{ route('fixtures.update', $fixture->id) }}" method="POST">
                    ...
                    <a href="{{ route('tournaments.show', $fixture->tournament_id) }}" class="btn-goback" style="display: inline-block; margin-top: 15px; text-align: center; width: 100%;">Annuleren</a>
                </form>
